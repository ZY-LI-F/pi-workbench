...
该命令使用真实协议、配对存储和 WebSocket Gateway，只把 Board 数据源替换为可预测的验收快照。它会打印一个 `stella://pair?...` 配对链接；在 Android App 中粘贴链接，或使用桌面 Stella 设置页生成的二维码完成配对。Android 模拟器默认通过 `10.0.2.2:43822` 连接宿主机；可用 `STELLA_COMPANION_ACCEPTANCE_ADDRESS` 和 `STELLA_COMPANION_ACCEPTANCE_PORT` 覆盖。
