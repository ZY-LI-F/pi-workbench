...
- [x] CONTEXT、README 和发布说明与实际 v0.5.0 行为一致。
- [x] package 与 lockfile 在全部功能验收后统一升级为 `0.5.0`。
- [x] `npm run check` 和确定性 Electron E2E 全部通过。
- [x] 本机 Codex/Claude 探测 smoke 不发送模型请求且结果明确。
- [x] macOS arm64 未打包与打包 smoke 通过，缺少外部 CLI 时仅对应能力降级。
- [x] 所有 tickets 的验收项和最终 Git 差异完成复核。
