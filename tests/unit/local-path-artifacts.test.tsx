...
describe("LocalPathArtifacts", () => {
  it("opens and copies a verified output directory", async () => {
    ...
    render(<LocalPathArtifacts api={api} text={`中间文件位置：\`${directoryPath}\``} />);
  ...
  it("opens a document or reveals its containing folder", async () => {
    const user = userEvent.setup();
    const api = desktopApi();
    render(<LocalPathArtifacts api={api} text={`成品：\`${presentationPath}\``} />);
  ...
  it("renders the output path affordance only for assistant results", async () => {
    ...
    const view = render(<MessageCard api={api} message={assistant} toolExecutions={{}} onFork={vi.fn()} />);
    expect(await screen.findByRole("region", { name: "输出文件与路径" })).toBeTruthy();

    view.rerender(<MessageCard api={api} message={{ role: "user", content: directoryPath, timestamp: Date.now() }} toolExecutions={{}} onFork={vi.fn()} />);
