...
import { formatSavingsCount, renderSolPiTool, showSolPiSavings } from "../../tui";
...
export function createObservationPackExtension(onActivity?: (message: string, warning?: boolean) => void): ExtensionFactory {
	return (pi: ExtensionAPI) => {
		const sentCounts = new Map<string, number>();
		pi.on("session_start", () => sentCounts.clear());
		pi.on("session_tree", () => sentCounts.clear());
		...
		pi.on("context", async (event, ctx: ExtensionContext) => {
			...
			for (let index = 0; index < event.messages.length; index += 1) {
				...
				try {
					...
					onActivity?.(`已投影 ${observation.id}；原文 ${observation.bytes} bytes 可召回（非实测计费节省）`);
					sentCounts.set(sendCountKey, previousSends + 1);
				} catch (error) {
					// Fail open: a packing failure must never cost the agent its observation.
					const reason = error instanceof Error ? error.message : String(error);
					console.error(`[observationpack] original result retained: ${reason}`);
					onActivity?.(`未应用日志投影，保留原始结果：${reason}`, true);
				}
			}
