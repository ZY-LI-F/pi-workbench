...
import { resolveToolPath } from "./file-queue";
import { renderSolPiTool, showSolPiSavings } from "../../tui";
import {
	createThenRunSchema,
...
export interface ActionFusionOptions {
	readonly beforeThenRun?: (command: string, ctx: ExtensionContext) => Promise<void>;
	...
	return (pi: ExtensionAPI) => {
		...
		pi.registerTool<typeof editParameters, EditToolDetails | undefined>({
			...
			async execute(toolCallId, input, signal, onUpdate, ctx) {
				...
				if (then_run) await options.beforeThenRun?.(then_run.command, ctx);
				if (signal?.aborted) throw new Error("Action Fusion cancelled before file mutation");
				...
				if (
					then_run &&
					result.content.some((block: { type: string; text?: string }) => block.type === "text" && block.text?.includes(THEN_RUN_SUCCEEDED))
				) {
					showSolPiSavings(ctx, "Action Fusion", "1 model round-trip avoided");
				}
				if (then_run) options.onActivity?.("文件修改和后续命令均已完成");
				return result;
			},
		...
		pi.registerTool<typeof writeParameters, undefined>({
			...
			async execute(toolCallId, input, signal, onUpdate, ctx) {
				...
				if (then_run) options.onActivity?.("文件写入和后续命令均已完成");
				return result;
			},
			renderCall: (args, theme, context) => {
...
export type { ThenRunInput } from "./then-run";
export {
