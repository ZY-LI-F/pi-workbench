...
import { formatSavingsCount, showSolPiSavings } from "../../tui";
import {
	DEFAULT_COMPACTION_ECONOMICS,
	decideCompaction,
	type CompactionDecision,
} from "./economics";
import { analyzePlanTransition, formatPlanSnapshot, parsePlanSteps } from "./plan";
import {
	appendOnlineState,
	initialOnlineState,
	recordBoundary,
...
import { registerOnlineTools, type PlanUpdateInput } from "./tools";
...
export type OnlineContextCompactOptions = {
	...
	readonly onActivity?: (message: string, warning?: boolean) => void;
	readonly cacheWriteReadRatio?: number | null;
	readonly keepRecentTokens?: number;
};

type PendingBoundary = { readonly toolCallId: string };
type SelectedCompaction = { readonly decision: CompactionDecision; readonly cancellationRevision: number };
...
export function nativeCompactableTokens(entries: readonly SessionEntry[], keepRecentTokens: number): number {
	...
	const prefixMessages =
		cut.isSplitTurn && cut.turnStartIndex >= 0
			...
			: 0;
	if (historyMessages === 0 && prefixMessages === 0) return 0;
	return path.slice(startIndex, cut.firstKeptEntryIndex).flatMap(sessionEntryToContextMessages)
...
export function createOnlineContextCompactExtension(options: OnlineContextCompactOptions = {}): ExtensionFactory {
	...
	return (pi) => {
		...
		pi.on("turn_end", (event, context) => {
			...
			const archiveTokens = nativeCompactableTokens(context.sessionManager.getBranch(), keepRecentTokens);
			...
			const decision: CompactionDecision =
				priced.compact && archiveTokens <= 0
					...
					: priced;
			if (!decision.compact) {
				options.onActivity?.(`阶段计划已保存，暂不压缩：${decision.reason}；合法前缀约 ${archiveTokens} tokens`);
				return;
			}

			selected = { decision, cancellationRevision: options.cancellationRevision?.() ?? 0 };
			context.abort();
		});
		...
		pi.on("agent_settled", async (_event, context) => {
			...
			if (!pending || pending.cancellationRevision !== (options.cancellationRevision?.() ?? 0)) {
			...
			try {
				...
				options.onActivity?.("阶段边界：正在调用 Pi 原生压缩；策略估算不是实测费用节省");
				await new Promise<void>((resolve) => {
					...
					context.compact({
						...
						onError: (error) => {
							...
							options.onActivity?.(`阶段压缩未完成，不会续跑：${error.message}`, true);
							finish();
						},
					});
				});
				...
				if (compacted && pending.cancellationRevision === (options.cancellationRevision?.() ?? 0)) {
					options.onActivity?.("Pi 原生压缩完成，正在继续当前任务");
