...
import { recordValue } from "./config";
import { openArchive } from "../../../secure-files";
...
async function exactBodyFromInline(inline: string, details: unknown): Promise<string> {
	...
	if (!(await safePiBashTempPath(candidate))) throw new Error("EPR 无法验证完整诊断日志；保留原工具结果");
	const handle = await openArchive(candidate, "read");
