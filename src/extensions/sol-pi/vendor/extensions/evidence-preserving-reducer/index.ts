...
import { formatSavingsBytes, showSolPiSavings } from "../../tui";
import { archiveBody, archiveRoot } from "./archive";
import { reducibleToolResult } from "./candidate";
import {
	DIAGNOSTIC_COMMAND,
	isRecord,
	LIKELY_SECRET,
...
import { callReducer, type ProviderResult } from "./provider";
import { receiptText, validateReceipt } from "./receipt";
...
export type EvidencePreservingReducerOptions = ReducerConfigOptions & { readonly onError?: (error: string) => void };
...
export function createEvidencePreservingReducerExtension(options: EvidencePreservingReducerOptions = {}): ExtensionFactory {
	return (pi: ExtensionAPI) => {
		...
		pi.on("tool_result", async (event, context) => {
			...
			return await reduceToolResult(state.journal, state.config, event, context);
			} catch (error) {
				options.onError?.(error instanceof Error ? error.message : String(error));
				console.error("[sol-epr] original tool result retained", error);
				return undefined;
			}
		});
	};
...
export type { ArchiveObject } from "./archive";
export {
	DIAGNOSTIC_COMMAND,
	loadReducerConfig,
...
export { validateReceipt } from "./receipt";
